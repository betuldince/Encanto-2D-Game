...
public class BodySourceView3 : MonoBehaviour
{
    ...
    public static BodySourceView3 Instance { get; private set; }
    ...
    void Update()
    {
        ...
        if(ClosestBody != null)
        {
            Debug.Log(ClosestBody.TrackingId);
            minDist = ClosestBody.Joints.GetValueOrDefault(JointType.SpineMid).Position.Z;
        }
        ...
        foreach (var body in data)
        {
            ...
            if (body.IsTracked)
            {
                ...
                Joint j = body.Joints.GetValueOrDefault(JointType.SpineMid);
                
                if(j.Position.Z <= minDist)
                {
                    ClosestBody = body;
                    minDist = j.Position.Z;
                }
                //Vector3.Distance(Camera.main.transform, );
                UpdateBodyObject(ClosestBody, mBodies[body.TrackingId]);
            }
        ...
        GameObject[] bodyObj;
        Transform [] t1;
        bodyObj = GameObject.FindGameObjectsWithTag("bodyObject");
        int i=0;
        foreach (GameObject b in bodyObj)
        {
            t1[i] = b.transform.GetComponentInChildren();
